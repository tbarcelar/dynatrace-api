...
output_path = 'D:/dyna/api/analise/extensao/excel/status-oracle.xlsx'

# Parâmetros da solicitação
params = {
    'from': '2024-10-20T12:45:00-03:00',
    'to': '2024-10-22T13:16:00-03:00',
    'limit': '1000',
    'query': 'event.type="sfm" AND dt.extension.name="com.dynatrace.extension.sql-oracle"',
    'sort': '-timestamp',
}
...
def fetch_logs(url, token, params):
    headers = {
        ...
        'Authorization': f'Api-Token {token}', # scope Read audit logs  e  Read logs
    }
...
def process_logs(data, name):
    ...
    for result in data['results']:
        ...
        combination = (config_id, content, status)
        if combination not in unique_combinations:
            unique_combinations.add(combination)
            records.append({'Cliente': name, 'ID': config_id, 'Content': content, 'Status': status})
    return records
